...
# Script to set up parent-child relationships between overview and implementation issues
# Usage: ./scripts/setup-parent-child-relationships.sh

set -e

echo "🔗 Setting up parent-child relationships between overview and implementation issues..."
...
get_issue_node_id() {
    ...
    }' -f owner="markwoitaszek" -f repo="llm-multimodal-stack" -f issueNumber="$issue_number" | jq -r '.data.repository.issue.id // empty'
}
...
set_parent_relationship() {
    ...
    if [[ "$child_item_id" == "null" || -z "$child_item_id" ]]; then
        echo "⚠️  Issue #$child_issue is not in the project. Adding it first..."
        
        # Add issue to project first
        local child_issue_id=$(get_issue_node_id $child_issue)
        if [[ -z "$child_issue_id" || "$child_issue_id" == "null" ]]; then
            echo "❌ Could not find Issue #$child_issue. Skipping..."
            return 1
        fi
        ...
        }' -f projectId="$PROJECT_ID" -f contentId="$child_issue_id" > /dev/null
        
        # Get the new project item ID
        child_item_id=$(get_project_item_id $child_issue)
    fi
